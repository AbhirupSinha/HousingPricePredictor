...
class Predictor:
    ...
    def __init__(self):
        ...
        """
        self.regressor = DecisionTreeRegressor(min_samples_split = 4, random_state = 0, max_features = 'sqrt',
                                  presort = True, splitter = 'random', min_samples_leaf = 1)
        """
        self.regressor = RandomForestRegressor(n_estimators = 20, random_state = 0, max_features = 'sqrt',
    ...
    def getPredictedPrice(self, inp):
        ...
        price = self.regressor.predict(input_data)
        #print(type(price))
        #print(type(price[0]))
        if(price[0]%10 < 5):
...
if __name__=='__main__':
    predictor = Predictor()
    dataset = predictor.loadData()
    X_train, Y_train, X_test, Y_test = predictor.splitData(dataset)
    if(predictor.trainData(X_train, Y_train, X_test, Y_test)):
